...
- **`--full-refresh`** — `DROP MATERIALIZED TABLE` then recreate. Required to change `distributed_by`/`buckets`, which are fixed at creation.

**Config:** `distributed_by`, `buckets` (default 6), `with`, `start_mode` (`FROM_BEGINNING` | `FROM_NOW` | `RESUME_OR_FROM_BEGINNING`). `freshness_interval`, `refresh_mode`, and `partition_by` are not supported and raise a compile error.
