...
> **Warning — data loss**: dropping a materialized table (including via `--full-refresh`) permanently deletes the backing Kafka topic and all of its data. (The associated Schema Registry subjects are *not* deleted — contrary to current Confluent docs — and can block a same-name recreate with a different schema shape; see the last bullet below and [Switching materializations](#materialized-table).)
...
- With a **differently-shaped Schema Registry schema** — most commonly when replacing a dropped materialized table with a `table` model, since the MT registered a *keyed* schema (primary key and `DISTRIBUTED BY HASH` derived from its query) while a snapshot CTAS registers a *keyless* one — creation is rejected with "Cannot create table because the Schema Registry subject `<name>-value` doesn't match the existing one". Unlike the conditions above, this does **not** clear on its own: the subject outlives the drop indefinitely. Delete the lingering `<name>-key`/`<name>-value` subjects in Schema Registry and re-run, or give the model a different relation name (e.g. via `alias`). The adapter appends this guidance to the error.

If you drop relations outside dbt, note that a materialized table must be dropped with `DROP MATERIALIZED TABLE`: the server silently *accepts* a regular `DROP TABLE` against an MT but phantom-drops it — the catalog entry transiently disappears, same-name creates fail with "table already exists", and the MT later resurfaces intact. The never-deleted topic of a phantom-dropped MT can also **resurface in the catalog as an inferred regular table** under the model's name; the next plain run then fails with the materialization-switch guard error ("already exists as a regular table or view") — run with `--full-refresh` to drop the inferred table (which deletes the lingering topic) and recreate the materialized table. Never rely on `DROP TABLE` being rejected; check `IS_MATERIALIZED` in `INFORMATION_SCHEMA.TABLES` first, as the adapter does before every table drop.
