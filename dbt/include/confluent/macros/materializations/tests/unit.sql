{# This macro is customized so we don't try to call `fetchall`, but
rather use `fetchmany(expected_rows | length)` which works fine
with the streaming cursor. If snapshot queries are available,
