-- This is copied from the default. FLINK SQL does not support creating temporary tables with
-- `SELECT .. AS`, so we need to use normal tables and remove them during cleanup instead.
-- For the change, look for the comment that starts with "Since Confluent".
